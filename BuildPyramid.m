...
%  dictionary. Each sift descriptor is given a texton label (πρακτικά
%  με τον όρο texton αναφέρεται στα κέντρα του Λεξικού. Προσοχή όμως! 
%  Επειδή εδώ δεν αναφερόμαστε απλά σε BoF αλλά σε Spatial Pyramid Matching
%  όπου μας ενδιαφέρει η χωρική πληροφορία των χαρακτηριστικών, όταν λέμε
%  ότι σε έναν SIFT περιγραφέα αναθέτουμε ένα κέντρο (ή texton label)
%  αυτό σημαίνει ότι ο περιγραφέας συσχετίζεται με ένα συγκεκριμένο visual 
%  word αλλά η συσχέτιση αυτή λαμβάνει επίσης υπόψιν της την θέση του 
%  περιγραφέα μέσα στο spatial pyramid) corresponding to the appropriate 
%  dictionary bin. Finally the spatial pyramid is generated from these 
%  label lists.
%
%
...
%               by the algorithm. If this dir is the same as imageBaseDir 
%               the files will be generated in the same location as the 
%               image files
...
% params.maxImageSize:    the max image size. If the image is larger it 
%                         will be resampeled.

% params.dictionarySize:  size of descriptor dictionary (200 has been found 
%                         to be a good size)

% params.numTextonImages: number of images to be used to create the 
%                         histogram bins

% params.pyramidLevels:   number of levels of the pyramid to build

% canSkip: if true the calculation will be skipped if the appropriate data 
%          file is found in dataBaseDir. This is very useful if you just 
...
%  50 texton images are used to build the histogram bins, 3 pyramid
...
%% build the pyramid
if(saveSift)
    GenerateSiftDescriptors(imageFileList,imageBaseDir,dataBaseDir, ...
        params,canSkip,pfig);
end

%% Σχηματισμός του Λεξικού. Χρήση k-means
CalculateDictionary(imageFileList,imageBaseDir,dataBaseDir,'_sift.mat', ...
    params,canSkip,pfig);
%% Σχηματισμός Ιστογράμματος
BuildHistograms(imageFileList,imageBaseDir,dataBaseDir,'_sift.mat',params, ...
    canSkip,pfig);
%%
pyramid_all = CompilePyramid(imageFileList,dataBaseDir, ...
    sprintf('_texton_ind_%d.mat',params.dictionarySize),params,canSkip, ...
    pfig);
close(pfig);
end
