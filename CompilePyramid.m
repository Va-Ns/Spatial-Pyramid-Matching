...
%
% Generate the pyramid from the texton labels
%
...
%                denote the data file that contains the textons indices and 
...
pyramid_all = zeros(length(imageFileList), ...
    params.dictionarySize*sum((2.^(0:(params.pyramidLevels-1))).^2));
...
for f = 1:length(imageFileList)
    ...
    outFName = fullfile(dataBaseDir, sprintf('%s_pyramid_%d_%d.mat', ...
        baseFName, params.dictionarySize, params.pyramidLevels));
    ...
    %% load texton indices
    in_fname = fullfile(dataBaseDir, sprintf('%s%s', baseFName, ...
        textonSuffix));
    ...
    for i=1:binsHigh
        for j=1:binsHigh
            ...
            pyramid_cell{1}(i,j,:) = hist(texton_patch, ...
                1:params.dictionarySize)./length(texton_ind.data);
        end
    end
    ...
    pyramid = [pyramid pyramid_cell{params.pyramidLevels}(:)'... 
        .* 2^(1-params.pyramidLevels)];

    %% Επεξήγηση της μορφής των σταθμίσεων.

    % Η στάθμιση κάθε επιπέδου στο paper είναι πράγματι ίση με 
    % 1/2^(L-l), όπου L είναι ο συνολικός αριθμός επιπέδων και l είναι 
    % το τρέχον επίπεδο. Ωστόσο, το απόσπασμα κώδικα χρησιμοποιεί μια 
    % διαφορετική σύμβαση για την ευρετηρίαση των επιπέδων. Στον κώδικα, 
    % το λεπτότερο επίπεδο δεικτοδοτείται με 1 και το χονδροειδές επίπεδο 
    % με L, ενώ στην εργασία, το λεπτότερο επίπεδο δεικτοδοτείται με L και
    % το χονδροειδές επίπεδο με 0. 
    %
    % Για παράδειγμα, εάν L = 3, τότε το paper θα αντιστοιχούσε τα 
    % βάρη 1/8, 1/4 και 1/2 στα επίπεδα 0, 1 και 2 αντίστοιχα. 
    % Το απόσπασμα κώδικα θα αναθέσει τα βάρη 1/8, 1/4 και 1/2 στα επίπεδα 
    % 3, 2 και 1, αντίστοιχα. Για να επιτευχθεί αυτό, το απόσπασμα κώδικα 
    % χρησιμοποιεί τον τύπο 2^(-l) για τα πιο χονδροειδή επίπεδα 
    % (l = 1,...,L-1) και 2^(1-L) για το λεπτότερο επίπεδο (l = L). 
    % Αυτό είναι ισοδύναμο με τον τύπο 1/2^(L-l) που χρησιμοποιείται, αλλά 
    % με διαφορετικό σχήμα δεικτοδότησης.
    %%
...
outFName = fullfile(dataBaseDir, sprintf('pyramids_all_%d_%d.mat', ...
    params.dictionarySize, params.pyramidLevels));
