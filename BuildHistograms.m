...
%                 denote the data file that contains the feature textons 
%                 and coordinates. Its default value is '_sift.mat'.
%
% dictionarySize: size of descriptor dictionary (200 has been found to be
%                 a good size)
%
% canSkip:        if true the calculation will be skipped if the 
%                 appropriate data file is found in dataBaseDir. This is 
%                 very useful if you just want to update some of the data 
%                 or if you've added new images.
...
for f = 1:length(imageFileList)
    ...
    if(mod(f,100)==0 && exist('pfig','var'))
        sp_progress_bar(pfig,3,4,f,length(imageFileList), ...
            'Building Histograms:');
    end
    outFName = fullfile(dataBaseDir, sprintf('%s_texton_ind_%d.mat', ...
        baseFName, params.dictionarySize));
    outFName2 = fullfile(dataBaseDir, sprintf('%s_hist_%d.mat', ...
        baseFName, params.dictionarySize));
    ...
    if ndata <= batchSize
        ...
        % Αυτό που κάνει εδώ είναι ότι βρίσκει που κατατάσσεται το κάθε
        % keypoint της εκάστοτε εικόνας στο εύρος των κέντρων (0-200).
        dist_mat = sp_dist2(features.data, dictionary);
        
        % Εναλλακτική γραφή
        % [~,index] = pdist2(dictionary,features.data,'euclidean','Smallest',1);
        
        % Εντοπίζεται η ελάχιστη απόσταση και επιστρέφεται ο δείκτης της 
        % ανά στήλη
    ...
    %% Στο σημείο αυτό σχηματίζεται το BoF. 
...
outFName = fullfile(dataBaseDir, sprintf('histograms_%d.mat', ...
    params.dictionarySize));

% Μπορούμε αν θέλουμε να αποθηκεύσουμε και το ιστόγραμμα όλων των εικόνων,
% δηλαδή το BoF.
