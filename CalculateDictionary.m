...
% First, all of the sift descriptors are loaded for a random set of images. 
% The size of this set is determined by numTextonImages. Then k-means is 
% run on all the descriptors to find N centers, where N is specified by
% dictionarySize.
%
...
%                denote the data file that contains the feature textons and
%                coordinates. 
...
reduce_flag = 1;
ndata_max = 100000; %use 4% available memory if its greater than the default
...
outFName = fullfile(dataBaseDir, sprintf('dictionary_%d.mat', ...
...
%% Δημιουργία ενός ενιαίου πίνακα SIFT χαρακτηριστικών

% Στο σημείο αυτό, δημιουργούμε έναν ενιαίο πίνακα SIFT χαρακτηριστικών
% που προκύπτει από την κατακόρυφη τοποθέτηση των SIFT δεδομένων της 
% εκάστοτε δεικτοδοτούμενης εικόνας. Θυμίζουμε, ότι οι εικόνες 
% επιλέγονται τυχαία, οπότε το ίδιο ισχύει και για την τοποθέτηση των 
% χαρακτηριστικών.
for f = 1:params.numTextonImages    
    ...
    % Φορτώνουμε τα SIFT δεδομένα της εκάστοτε εικόνας.
    inFName = fullfile(dataBaseDir, sprintf('%s%s', baseFName, ...
        featureSuffix));
    if(exist(inFName,'file'))
        load(inFName, 'features');
    else

        % Αν δεν υπάρχουν, τα παράγουμε.
...
% Στο σημείο αυτό, γίνεται μια ελάττωση των χαρακτηριστικών σε περίπτωση
% που ισχύουν τα παρακάτω:
%                       1) Το reduce_flag = 1
%                       2) Το πλήθος των δεδομένων υπό επεξεργασία είναι
%                          μεγαλύτερο από τον αρχικό μας περιορισμό.
...
% Προσοχή στις διαστάσεις των κέντρων. Στην πρώτη διάσταση τοποθετούνταi
% τα κέντρα και στην δεύτερη τον αριθμό των χαρακτηριστικών, άρα εδώ θα 
% έχει διαστάσεις 200-by-128.
