function [centers, options, post, errlog] = sp_kmeans(centers, data, options)
...
%	Description:
%   -----------------------------------------------------------------------
%	CENTRES = KMEANS(CENTRES, DATA, OPTIONS) uses the batch K-means (η 
%   χρήση της λέξης batch εδώ δεν υποδηλώνει ότι τα δεδομένα επεξεργάζονται
%   ανά κομμάτια, αλλά όλα μαζί. Διαφορετικά θα μας έλεγε mini-batches)
...
%	ERRLOG. The optional parameters have the following interpretations.
%
...
for n = 1:niters
  ...
  for j = 1:ncentres
    if (num_points(j) > 0)
      centers(j,:) = sum(data(find(post(:,j)),:), 1)/num_points(j);
    end
  end
  ...
  if n > 1
    ...
    % Εδώ έκανα την αλλαγή από απλό & σε &&
    if max(max(abs(centers - old_centers))) < options(2) && ...
